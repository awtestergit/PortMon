...
// Define your own GUIDs using `uuidgen` or an online generator
DEFINE_GUID(GUID_PORTMON_CALLOUT, 0xbbbbbbbb, 0xcccc, 0xdddd, 0xee, 0xff, 0x00, 0x11, 0x22, 0x33, 0x44, 0x55);
...
NTSTATUS RegisterWfpCallout(PDEVICE_OBJECT DevObj) {
    ...
    MgrCallout.applicableLayer = FWPM_LAYER_ALE_AUTH_CONNECT_V4;
...
void NTAPI CalloutClassifyFn(
    ...
    UINT16 Protocol = InFixedValues->incomingValue[FWPS_FIELD_ALE_AUTH_CONNECT_V4_IP_PROTOCOL].value.uint16;
    // UINT16 LocalPort = ntohs(InFixedValues->incomingValue[FWPS_FIELD_ALE_AUTH_CONNECT_V4_IP_LOCAL_PORT].value.uint16);
    UINT16 LocalPort = RtlUshortByteSwap(InFixedValues->incomingValue[FWPS_FIELD_ALE_AUTH_CONNECT_V4_IP_LOCAL_PORT].value.uint16);
    UINT32 ProcessId = (UINT32)InMetaValues->processId;

    // TODO: identify port assignment / release
    BOOLEAN IsAssignment = (InFixedValues->layerId == FWPS_LAYER_ALE_AUTH_CONNECT_V4);
